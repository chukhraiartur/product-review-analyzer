...
def mock_gcs_service():
    ...
    # Mock methods
    mock_service.save_html.return_value = "https://storage.googleapis.com/test-bucket/test.html"
    mock_service.save_image.return_value = "https://storage.googleapis.com/test-bucket/test.jpg"
    mock_service.save_logs.return_value = "https://storage.googleapis.com/test-bucket/test.log"
...
def test_client():
    ...
    with patch("app.api.dependencies.get_gcs_service") as mock_gcs, \
         patch("app.api.dependencies.get_llm_service") as mock_llm, \
         patch("app.api.dependencies.get_vector_db_service") as mock_vector:
        
        # Setup mocks
        mock_gcs.return_value = Mock(spec=GCSService)
        mock_llm.return_value = Mock(spec=LLMService)
        mock_vector.return_value = Mock(spec=VectorDBService)
